...
|---|---|---|
| 0.3.0 | 2026-05-25 | Address TE v2 review (F-01–F-04) and PM v2 review (F-01): equity-passthrough integration test invariant (ADR-WHEEL-03), tmp_path isolation and integer-sort regression test (ADR-WHEEL-05), three-level fallback test coverage with level-2 freetext-JSON scenario (ADR-WHEEL-04), sentinel constant enforcement (ADR-WHEEL-04), zero-variance sentinel disclosure (ADR-WHEEL-01) |
...
- **Zero-variance sentinel disclosure:** When the zero-variance sentinel fires (`iv_rank = 50` due to a flat OHLCV lookback series), the formatted report string includes the note `'IV range is flat — rank set to neutral 50'`. The CLI output (REQ-SCREEN-03) and the `WheelCandidateReport.iv_assessment` field (REQ-SCREEN-02) must surface this note verbatim when it is present, so users are not misled by a sentinel-generated `iv_environment = "normal"` result. Because `iv_rank = 50` passes WheelAnalyst Criterion 1 (`iv_rank >= 25`), a sentinel-generated approval that originates from degenerate data must be clearly disclosed. This disclosure requirement is parallel to the realised-vol label requirement above and must be carried into the PLAN and FSPEC for WheelAnalyst and WheelCandidateReport (REQ-DATA-02, REQ-SCREEN-01, REQ-SCREEN-02).
...
- **Equity-passthrough integration test invariant:** A dedicated integration test must assert that when `wheel_phase is None`, the `wheel_router` passes the invocation through to `fundamental_analyst` (or the first analyst node) unchanged — `wheel_phase` remains `None` in the output `AgentState`, no options-layer tools are invoked, and the final output fields (`market_report`, `sentiment_report`, etc.) are functionally equivalent to the output produced by a pre-feature equity-only invocation (REQ-NFR-01). This property must appear in the PROPERTIES document and is the primary regression guard for LangGraph version bumps that affect `START` edge semantics. The Re-evaluation Trigger's reference to "the equity pass-through integration test" is forward-referencing this required test.
...
- **Sentinel constant enforcement:** The sentinel marker string must be defined as a module-level constant `STRUCTURED_OUTPUT_SENTINEL = 'Structured output failed — safe fallback applied.'` in `tradingagents/agents/utils/structured.py`. All four wheel agents and the CLI display layer must import and use `STRUCTURED_OUTPUT_SENTINEL` rather than hardcoding the string. Tests that assert on the sentinel `rationale` value must import `STRUCTURED_OUTPUT_SENTINEL` from `structured.py` rather than using a string literal, so that any accidental mutation of the constant surfaces as a test failure rather than a silent CLI rendering regression.
- **Test coverage:** The three-layer fallback requires three independent test scenarios per agent: (a) structured-output success — mock `invoke_structured_or_freetext` to simulate the structured path returning a valid Pydantic-rendered string and assert the agent parses it correctly without returning the sentinel; (b) freetext fallback succeeds — mock `invoke_structured_or_freetext` to return a valid bare JSON string matching the schema, assert `model_validate_json` parses it successfully and the sentinel is NOT returned (this is the guard against the agent silently treating a valid freetext JSON response as a total failure and returning unnecessary `tradeable=False` outcomes); (c) total failure — mock both paths to return unparseable content, assert the agent returns the sentinel with `tradeable=False`/`action='HOLD'` and a non-empty `rationale` equal to `STRUCTURED_OUTPUT_SENTINEL`. Scenario (b) is the level-2 freetext-valid-JSON path that covers the silent `tradeable=False` risk. The three scenarios apply to each of the four wheel agents, totalling 12 required fallback test cases. The sentinel `tradeable=False` / `action='HOLD'` must additionally be verified not to cause graph crashes downstream. A dedicated property 'Agent returns sentinel on total structured output failure' must appear in PROPERTIES.
...
- **I/O-layer test isolation:** Unit tests for the I/O-layer functions (`save_wheel_position`, `load_wheel_position`, `load_latest_open_position`) must use a pytest `tmp_path`-scoped directory and must never write to the configured `positions_dir`. The `_position_loader` injectable eliminates filesystem access at the agent level; the I/O-layer functions themselves require separate `tmp_path`-isolated tests. Without this discipline, test authors may inadvertently write to the real positions directory and produce test pollution that persists across runs. This requirement must appear in the PROPERTIES document.
- **Integer-sort regression test:** A dedicated regression test must assert that `load_latest_open_position` returns the cycle-10 position file and not the cycle-9 file when both are present in a `tmp_path`-scoped directory (integer sort invariant, TE-TSPEC-04). This test is the primary guard against re-introducing lexicographic sort order, which silently produces the wrong result at cycle ≥ 10 — `"cycle-9"` sorts after `"cycle-10"` character-by-character. This property must appear in the PROPERTIES document.
